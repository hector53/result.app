         ...
         <div class="cubreHoras" v-if="activarW" >
            ...
            v-for="(item2, index2) in time[index].horas" :key="index2"
             style="padding-left: 10px;    padding-right: 10px;    margin: 0; margin-bottom: 10px; display: flex;">
                         <b-field label="Hora Inicio" style="     padding-top: 10px; width:100%">
                             <b-timepicker :time-formatter="formatoHora" icon="clock" hour-format="24" 
                             placeholder="Hora Inicio"  :id="'tiempo_'+index" v-model="time[index].horas[index2].ini"   inline></b-timepicker>
                            </b-field>
                           <a v-if="index2 > 0"  class="close close_option" style="    margin-top: 55px;" @click="quitarHoras(index, index2)"     ></a>
                </div> 
              <button @click="agregarHoras(index)" class="btnAddHoras">Agregar Horas</button>
         </div>
...
d.setHours(0,0,0,0);
    this.time.push({id: this.convertDate(difference2), horas: [ {ini: d, fin: d}] }); 
    console.log(this.convertDate(difference2));
}else{
    console.log("quito la selecciion", this.convertDate(difference))
     const index =  this.time.findIndex( fruta => fruta.id === this.convertDate(difference) );
  ...
  methods: {
    ...
    extraerDia(fecha) {
      const monthNames = ["Enero", "Febrero", "Marzo", "Abril", "Mayu", "Junio",

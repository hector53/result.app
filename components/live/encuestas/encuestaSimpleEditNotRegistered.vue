  ...
  <section class="section-hero" style="padding-top: 20px">
    ...
    <div class="container">
      <div class="form-block w-form has-text-left">
        ...
        <div
          class="divR"
          v-for="(item, index) in opcionEncuestaNuevo"
          :key="index + 100"
        >
          <label
            :for="'opcion' + (opcionEncuesta.length + index)"
            class="field-label has-text-left"
            v-text="
              $store.state.idioma.optionLabel +
              ' ' +
              (cantidadOpcionesViejas + (index + 1))
            "
          ></label>
          <input
            type="text"
        ...
        <div class="text-block-5" @click="addOpcion">
...
export default {
  ...
  methods: {
    ...
    async crearEncuesta(val) {
      ...
      if (
        this.opcionEncuesta.length == 0 &&
        this.opcionEncuestaNuevo.length == 0
      ) {
        this.$swal({
          type: "error",
          title: "Oops...",
      ...
      await this.$axios
        .$post("edit_poll_simple_user_not_registered", {

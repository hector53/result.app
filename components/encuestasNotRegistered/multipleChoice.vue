  ...
  <div>
    <section class="section-hero" style="min-height: 500px; padding: 5px">
      <div class="container cubreEncuesta">
        ...
        <p
          class="hero-subhead has-text-left"
          v-text="
            $store.state.idioma.createdAbout +
            ' ' +
            fecha +
            ' por Anonimo ' +
            totalVotos +
        ...
        <div class="mt-5">
          <div class="w-form" style="padding: 10px">
                  ...
                  <div class="column is-flex is-justify-content-space-between">
                    <div class="cubreIconandOption is-flex">
                      ...
                      <div class="div-block-8">
                        <div
                          class="div-block-9"
                          v-if="item['porcentaje'] > 0 && miVoto == item['id']"
                        ></div>
                      </div>
                      <span
                        for="Opción "
            ...
            <div class="div-block-10">
              <button class="buttonN blue">
                {{ $store.state.idioma.notificationEmail }}
              </button>
              ...
              <a
                class="link-2"
                @click="cancelarVoto"
                v-if="resultados && yaVote"
                >{{ $store.state.idioma.cancelVote }}</a
              >
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async cancelarVoto() {
      ...
      await this.$axios
        .$get(
          "_cancelar_voto_not_registered?id_encuesta=" +
            this.id_encuesta +
            "&u=" +
            cookieNotUser +
            "&codigo=" +
    ...
    async getEncuesta(id) {
      ...
      await this.$axios
        .$get(
          "_get_encuesta_not_registered?codigo=" + id + "&u=" + cookieNotUser
        )
        .then((response) => {
          console.log(response);
          if (response != 0) {
    ...
    detectaTecla(event) {
      ...
      for (var i = 0; i < 9; i++) {
        if (
          event.keyCode === tecladoArriba + i ||
          event.keyCode === tecladoDerecho + i
        ) {
          //presiono la tecla 1
